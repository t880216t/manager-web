...
class InterfaceList extends React.Component{
    ...
    fetchList=()=>{
        var par = ""
        fetch('http://192.168.0.1:5000/interfaceTaskList',{
            method: "POST",
            mode: "cors",
            headers: {
    ...
    fetchScript =(entry)=>{
        var par = "entry="+entry
        fetch('http://192.168.0.1:5000/build',{
            method: "POST",
            mode: "cors",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
    ...
    getReportUrl=(entry,task_status)=>{
        const url = "http://192.168.0.1:5000/static/interface_auto/test_src/"+entry+"/result/result.html"
        if (task_status === 3){
