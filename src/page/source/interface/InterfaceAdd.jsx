...
class InterfaceAdd extends React.Component{
    state = {
        keyInput :'',
        project :'',
        test_name:'',
    ...
    //格式化上传参数，提交数据
    handleSubmit =()=>{
        if(this.state.project != '' && this.state.test_name != '' && this.state.path != ''&& this.state.verif_code.toString() != '' && this.state.test_description != ''){
            this.setState({
                isLoading:true,
            },()=>{this.fetchAdd()})
        }else {
    ...
    fetchAutoCompleteWords = () =>{
        ...
        fetch('http://ownerworld.win:5000/getAutocompleteWords',{
            method: "POST",
            mode: "cors",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
            ...
            .then((responseData) => { // 上面的转好的json
                if (responseData.code === 0) {
                    ...
                    },()=>{console.log('complete data :',this.state.autoCompleteItems)})
                }
                else {
    ...
    render(){
        const { method,project } = this.state;
        const parms_items =this.state.parms.split("|~|")
        return(
            <Page title="新建用例数据" loading={this.state.isLoading} >
                <div style={{flex:1,display:'flex',justifyContent:'flex-end',marginRight:50,}}>
                    <Button style={{marginLeft:10,marginBottom:20,backgroundColor:'#49a9ee'}} onClick={this.handleSubmit}>提交</Button>
                </div>
                <div style={{marginBottom:100}}>
                    <div style={{marginLeft:50,marginRight:50}}>
                        ...
                        <div style={styles.input_box}>
                            ...
                            <div style={styles.input_box_back}>
                                <Input
                                    ...
                                    onChange = {(value)=>{this.setState({test_name:value.target.value.replace(/\//g,'_')})}}
                                    style={{ width: '60%' }}
                        ...
                        <div style={styles.input_box}>
                            ...
                            <div style={{display:'flex',flexDirection:'row',flex:6}}>
                                <div style={{marginLeft:10,padding:3}}>
                                    <Switch onChange={(e)=>this.isNeedSaveResponse(e)}></Switch>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={styles.input_box}>
                            ...
                            <div style={{display:'flex',flexDirection:'row',flex:6}}>
                                <div style={{marginLeft:10,padding:3}}>
                                    <Switch onChange={(e)=>this.isNeedVerifResponse(e)}></Switch>
                                </div>
                            </div>
                        </div>
                        ...
                        {this.state.need_verif_value === '1'?
                            <div style={styles.input_box}>
                                ...
                                <div style={{display:'flex',flexDirection:'row',marginTop:10}}>
                                    ...
                                    <div style={{marginLeft:10,padding:3}}>
                                        <Switch onChange={(e)=>this.isNeedVerifFromFile(e)}></Switch>
                                    </div>
